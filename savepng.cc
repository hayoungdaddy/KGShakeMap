...
int SDL_SavePNG_RW(SDL_Surface *surface, SDL_RWops *dst, int freedst) 
{
  ...
  if (freedst) {
    // Lim for close 
    SDL_RWclose(dst);        // http://sdl.beuc.net/sdl.wiki/SDL_RWclose
    //SDL_FreeRW(dst);       <-- SDL_FreeRW :: Only use it on memory allocated by SDL_AllocRW. 
    //                           Any extra memory allocated during creation of the RWops is not freed by SDL_FreeRW
    //                           the programmer must be responsible for it.
  }
